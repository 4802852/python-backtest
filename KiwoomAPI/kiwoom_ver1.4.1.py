...
class MyWindow(QMainWindow):
    ...
    def run(self, symbol_list):
        ...
        self.timer.timeout.connect(self.timeout_run_time)

        self.timer_notifier = QTimer(self)
        notify_interval = 1000 * 60 * 60
        self.timer_notifier.start(10000)
        self.timer_notifier.setInterval(notify_interval)
        self.timer_notifier.timeout.connect(self.timeout_run_notifier)
    ...
    def sell_all(self, bought_list):
        self.ocx.OnReceiveRealData.disconnect()
        today = self.get_today()
        self.plain_text_edit.appendPlainText("전량 매도 진행")
        ...
        self.bought_list = set([])
        self.sleep(15000)
        self.plain_text_edit.appendPlainText("전량 매도 완료")
        self.ocx.OnReceiveRealData.connect(self._handler_real_data)
