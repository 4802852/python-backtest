...
def backtest(money: int, fee_rate: float, interval: int, etf_names :arrays, etf_codes :arrays, etf_rates :arrays, etf_colors :arrays, start_date: str):
    ...
    for each in new_df.index:
        ...
        for i in range(1, len(etf_codes) + 1):
            etf_price = new_df[f'etf{i}'][each]
            money, etf_num_tmp, etf_money_tmp = buy_etf(money, etf_price, etf_num[i], fee_rate, etf_rates[i - 1])
    ...
    print(backtest_df)
    backtest_df.to_csv('data_output.csv', mode='w')
    percentage(interval)
